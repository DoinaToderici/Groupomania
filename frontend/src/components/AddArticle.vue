...
<div class="add-post-page  container">
  <div class="add-post-form col-lg-8 col-sm-12">
    <h1>Veuillez écrire  votre article</h1>
    <div>
      ...
      <button @click="savePost" class="btn cta-success col-lg-3 col-md-4 col-sm-12 m-auto d-flex justify-content-center">Enregistrer</button>
    </div>
  </div>
  </div>
</template>
  ...
  methods: {
    validateUserData() {
      ...
      if (this.post.description.length === 0) {
        errors.description = 'Veuillez remplir tous les champs'
      }
      ...
      PostDataService.create(data)
        .then((response) => {
          console.log(response);
            this.submitted = true;
            this.post = {};
            this.legacySystemHTML = `Votre article a été crée avec succès <em><a href="http://localhost:8080/post/${response.data.post.id}">Voir l'article</a><em>`
